...
### Improved
- **MCP agent guidance**: Improved query help documentation, filter syntax examples, timestamp usage guidance, pagination percentage display, and partial-result warnings.
...
### Added
- **MCP server (experimental) **: Integrated MCP (Model Context Protocol) server via rmcp with Streamable HTTP transport at `/mcp`. Provides 10 tools for AI agents: `system_overview`, `query_data`, `query_count`, `query_help`, `integrity_report`, `scan_history`, `browse_filesystem`, `search_files`, `item_detail`, `scan_changes`. Compatible with Claude Desktop (via mcp-remote), Claude Code, and other MCP clients.
...
- **Independent scroll regions on Browse page**: Tree/folder/search and item detail panel scroll independently, preventing detail panel from scrolling out of view. Detail panel always visible with empty state placeholder
