...
The Dashboard is the home page of fsPulse. It answers the most important question at a glance: **"Is my data safe?"**

<!-- Screenshot: Dashboard page showing root health cards, active task, and upcoming tasks -->
<!-- ![Dashboard Overview](screenshot-placeholder-dashboard-overview.png) -->

---

## Root Health Summary

The centerpiece of the Dashboard is the root health summary, which shows the status of every monitored directory:
...
When no roots have been configured, the Dashboard shows a welcome message with a link to [Setup](setup.md) to add your first root.
...
When a scan or other task is running, the Dashboard displays a progress card showing:
...
Progress updates are delivered in real time via WebSocket. When no task is running, the card shows an idle state with a button to initiate a manual scan.
