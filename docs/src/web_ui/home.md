...
<!-- Screenshot: Root health summary -->
...
A compact summary of recent scan and task activity, showing the last several completed operations with their outcomes. Each entry shows change counts (adds, modifications, deletions) and an **Integrity** column that links to the [Integrity](integrity.md) page showing any new validation errors or suspect hashes detected during that scan. For the full activity log, click the **View All** link to navigate to the [History](history.md) page.
