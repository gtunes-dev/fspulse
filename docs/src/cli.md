...
- Provides a guided menu for reporting and querying
- Allows interactive query mode with history
- Works with roots and scans created via the web UI
...
**Scanning is performed exclusively through the web UI.** Start the server with `fspulse serve` and access the web interface to:
- Create and manage scan schedules
- Initiate manual scans with real-time progress monitoring
- Configure scan options (hashing, validation)

The CLI commands documented on this page provide access to data generated by scans, but cannot initiate scans themselves.
