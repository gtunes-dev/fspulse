...
// Web API-friendly scan record structure
...
impl ScanRecord {
    pub fn list_paginated(
        db: &Database,
        query: &ListQuery,
    ) -> Result<ListResult<ScanRecord>, FsPulseError> {
        // First get the total count for pagination
        let count_sql = if let Some(filter) = &query.filter {
            format!(
                "SELECT COUNT(*) FROM scans s
                 JOIN roots r ON s.root_id = r.root_id
                 WHERE r.root_path LIKE '%{}%' OR
                       CASE s.state
                         WHEN 0 THEN 'pending'
                         WHEN 1 THEN 'scanning'
                         WHEN 2 THEN 'sweeping'
            ...
            "SELECT COUNT(*) FROM scans".to_string()
        };
        ...
        let order_clause = match query.sort.as_deref() {
            Some("id") => "ORDER BY s.scan_id ASC",
            Some("id_desc") => "ORDER BY s.scan_id DESC",
            Some("scan_time") => "ORDER BY s.scan_time ASC",
            Some("scan_time_desc") => "ORDER BY s.scan_time DESC",
            Some("state") => "ORDER BY s.state ASC",
            Some("state_desc") => "ORDER BY s.state DESC",
            Some("root_path") => "ORDER BY r.root_path ASC",
            Some("root_path_desc") => "ORDER BY r.root_path DESC",
            Some("file_count") => "ORDER BY s.file_count ASC",
            Some("file_count_desc") => "ORDER BY s.file_count DESC",
            Some("folder_count") => "ORDER BY s.folder_count ASC",
            Some("folder_count_desc") => "ORDER BY s.folder_count DESC",
            _ => "ORDER BY s.scan_time DESC", // Default: newest first
        };
        ...
        let main_sql = format!(
            "SELECT s.scan_id, s.root_id, r.root_path,
                    CASE s.state
                      ...
                      ELSE 'unknown'
                    END as state,
                    datetime(s.scan_time, 'unixepoch') || 'Z' as scan_time,
                    s.file_count,
                    s.folder_count
             FROM scans s
             JOIN roots r ON s.root_id = r.root_id
        ...
        let scan_iter = stmt.query_map([], |row| {
            Ok(ScanRecord {
                id: row.get(0)?,
                root_id: row.get(1)?,
                root_path: row.get(2)?,
